...
import {
  LayoutDashboard, User, DollarSign, Receipt, Shield, MessageSquare, FileOutput,
...
function AppContent() {
  ...
  useEffect(() => {
    if (profiles.length > 0 && profileId === 0) {
      setProfileId(profiles[profiles.length - 1].id);
    }
  }, [profiles, profileId]);

  // Auto-create a 2025 profile if none exist
  useEffect(() => {
    if (profiles.length === 0 && profileId === 0) {
  ...
  const renderPage = () => {
    ...
    switch (activeTab) {
      ...
      default: return <Dashboard profileId={profileId} />;
    }
  };
    ...
    <div className="flex h-screen bg-background">
      ...
      <aside className={`${collapsed ? "w-16" : "w-60"} flex-shrink-0 border-r bg-sidebar transition-all duration-200 flex flex-col`}>
        ...
        <div className="p-2 border-t space-y-1">
          <button onClick={() => setDark(!dark)} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-muted-foreground hover:bg-accent hover:text-foreground transition-colors" data-testid="button-theme-toggle">
            {dark ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}
            {!collapsed && <span>{dark ? "Light Mode" : "Dark Mode"}</span>}
          </button>
          <button onClick={() => setCollapsed(!collapsed)} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-muted-foreground hover:bg-accent hover:text-foreground transition-colors" data-testid="button-collapse-sidebar">
        ...
        <div className="max-w-5xl mx-auto p-6">
          <div className="mb-6">
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              Tax Year {profiles.length > 0 ? profiles[profiles.length - 1].taxYear : 2025} — Married Filing Jointly
            </p>
          </div>
          {renderPage()}
        </div>
